...
class BaseAviary(gym.Env):
    ...
    def _randomizer(self, init_seed = 1):
        ...
        base_color =  [str(round(random.uniform(0,1), 1)), str(round(random.uniform(0,1), 1)), str(round(random.uniform(0,1), 1)), 1.0]
        ...
        #### Desired velocity ######################################
        self.gv_velocity = 48 - 24*np.random.rand()
        #self.gv_velocity = 18 - 6*np.random.rand()
        # self.gv_velocity = 1
        #### Max force to reach the desired velocity ###############
        self.gv_force_limit = 600
        ...
        #lines = open(self.urdf_file, 'r')
        #for line in lines.readlines()[0:20]:
        #    print(line)
    ...
    def _getDroneImages(self,
        ...
        DRONE_CAM_VIEW = p.computeViewMatrix(cameraEyePosition=self.pos[nth_drone, :] - np.array([0, 0, 0.05]) +np.array([0, 0, self.L]),
                                             cameraTargetPosition=target,
                                             cameraUpVector=[1, 0, 0],
                                             physicsClientId=self.CLIENT
                                             )
